...
@section('title', 'تعديل فرقة دراسية')
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">تعديل الفرقة الدراسية: {{ $level->name }}</h5>
        </div>
        <form action="{{ route('levels.update', $level->id) }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <div class="row g-3">
                        @php
                            $selectedSections = $level->sections->pluck('id')->toArray();
                        @endphp
                                ...
                                <div class="form-check custom-option custom-option-basic">
                                    <label class="form-check-label custom-option-content" for="section_{{ $section->id }}">
                                        <input class="form-check-input" type="checkbox" name="section_ids[]"
                                               ...
                                               {{ is_array(old('section_ids', $selectedSections)) && in_array($section->id, old('section_ids', $selectedSections)) ? 'checked' : '' }}>
